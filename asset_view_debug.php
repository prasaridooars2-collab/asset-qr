...
/* ============================================================
   ...
   !!! SECURITY SETUP (InfinityFree) !!!
   InfinityFree shudhu "htdocs" folder-e file upload allow kore,
   tai DB password-soho "db_config.php" file-ta o "htdocs"-er
   bhitorei thake — kintu "htdocs"-er nijer ".htaccess" file
   diye eta web-browser theke direct access kora bondho kora
   ache (Deny from all).

   Apnar File Manager-e structure emon hote hobe:

       htdocs/
         ├── .htaccess          <-- db_config.php-ke block kore
         ├── db_config.php      <-- password ekhane (protected)
         └── asset_view.php     <-- ei file
   ============================================================ */

// ---- DEBUG VERSION: error screen-e dekhabe, jate problem ber kora jay ----
ini_set('display_errors', '1');
error_reporting(E_ALL);

// ---- db_config.php load kora (eki "htdocs" folder-er bhitore,
//      .htaccess diye web-access theke protected) ----
$configPath = __DIR__ . '/db_config.php';

if (!file_exists($configPath)) {
    http_response_code(500);
    error_log("asset_view.php: db_config.php paowa jay nai. Path: " . $configPath);
...
if (!$DB_PASS || $DB_PASS === "REAL_AIVEN_PASSWORD_DIN_EKHANE") {
    error_log("asset_view.php: db_config.php-e real password set kora hoyni.");
    ...
    // InfinityFree-er shared hosting-e shadharonoto custom CA file
    // upload kora jay na, tai amra encrypted connection rakhchi
    // kintu certificate verification off rakhchi (Aiven nije
    // SSL force kore, tai data tobuo encrypted thakbe).
    ...
    if (!$connected) {
        error_log("asset_view.php DB connection failed: " . mysqli_connect_error());
        ...
        if ($stmt) {
            ...
            error_log("asset_view.php prepare() failed: " . $mysqli->error);
        }
        $mysqli->close();
...
<?php else: ?>
    ...
    <div class="card">
        ...
        <div class="body">
            ...
            <div class="grid">
                ...
                <div>
                    ...
                    <div class="field-value">
                        <?php echo $asset['invoice_date'] ? e(date("d-M-Y", strtotime($asset['invoice_date']))) : "—"; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="value-highlight">৳ <?php echo number_format((float)$asset['asset_value'], 2); ?></div>

            <div class="footer-note">
                Asset ID: <?php echo e($asset['asset_id']); ?> &nbsp;|&nbsp; Scanned via QR Code
            </div>
        </div>
    </div>
